...
//        return $crypt_str;
    ...
    {
        //个人零钱总额
        $money_bag_total = Member::query()->where('type','!=',40)->sum('money_bag');
        //商家营业款总额
        $shop_money_total = Member::query()->where('type',40)->sum('revenue_money');
        //个人积分总数
        $integral_total = Member::query()->where('type','!=',40)->sum('total_credits');
        //商家积分总数
        $shop_integral_total = Member::query()->where('type',40)->sum('total_credits');
        return view('admin.main',[
            'money_bag_total' => $money_bag_total,
            'shop_money_total' => $shop_money_total,
            'integral_total' => $integral_total,
            'shop_integral_total' => $shop_integral_total,
        ]);
    }
